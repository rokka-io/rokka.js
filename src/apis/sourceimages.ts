...
/**
 * C2PA / Content Credentials of an image, read from its embedded manifest.
 *
 * The whole `content_credentials` key is omitted from `static_metadata` when
 * the image has no readable manifest at all. A block with `present: false`
 * means a manifest was looked for, but couldn't be validated - those two
 * states are not the same, so check with `?.content_credentials?.present`.
 */
export type ContentCredentials = {
  /** Whether the image carries a valid C2PA manifest */
  present: boolean
  /**
   * Whether the manifest declares the image as AI generated, ie its
   * `digital_source_type` is IPTC's `trainedAlgorithmicMedia`
   */
  ai_generated: boolean
  /** The IPTC digital source type URI of the `c2pa.created` action */
  digital_source_type: string | null
  /** The producing software agent, eg `gpt-image` */
  generator: string | null
  /** The cryptographic signer of the manifest, eg `OpenAI OpCo, LLC` */
  signed_by: string | null
  /** The overall validation state of the manifest, eg `Valid` */
  validation_state: string | null
}

/**
 * The `static_metadata` of a source image, filled by rokka on upload.
 *
 * Only `content_credentials` is typed, all the other blocks (`exif`, `webp`,
 * ...) stay untyped. `Sourceimage.static_metadata` itself is still `any` for
 * backwards compatibility, cast it to this type to get the typing:
 * `image.static_metadata as StaticMetadata | undefined`
 */
...
export interface SourceimageWithStaticMetadata extends Sourceimage {
  static_metadata?: StaticMetadata
}

export interface ContentCredentialsResponse extends RokkaResponse {
  body: SourceimageWithStaticMetadata
}
...
export class SourceimagesApi {
   ...
   * ```
   *
   * @example Searching in static metadata, eg for AI generated images
   * ```js
   * const search = {
   ...
   * generated, ChatGPT/DALL-E images for example embed such a signed manifest.
   *
   * rokka reads them automatically on every upload, so you usually don't need
   * this. It re-reads the original uploaded file (re-encoding an image strips
   * its manifest) and refreshes the `content_credentials` static metadata
   * block, which is useful after rokka's detection improved.
   *
   * ```js
   ...
   * ```
   *
   * You can also search for it, see {@link list}. Note that the search values
   * have to be strings, `'true'` and not `true`:
   *
   * ```js
   * const result = await rokka.sourceimages.list('myorg', {
   ...
   *
   * @remarks
   * Requires authentication and the `sourceimages:write` role.
   *
   * @since 4.2.0
   * @param organization - Organization name
   * @param hash - Image hash
   * @returns Promise resolving to the source image with refreshed content credentials
   */
  contentCredentials(
    organization: string,
    hash: string,
  ): Promise<ContentCredentialsResponse> {
    return this.state.request(
      'POST',
      `sourceimages/${organization}/${hash}/contentcredentials`,
    )
  }
